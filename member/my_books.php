...
<html>
	...
	<body>
		...
		<?php
			$query = $con->prepare("SELECT book_id FROM book_issue_log WHERE member = ?;");
			...
			else
			{
				...
				for($i=0; $i<$rows; $i++)
				{
					...
					if($ID != NULL)
					{
						$query = $con->prepare("SELECT title, author, category FROM book WHERE id_titulo = ?;");
			...
			if(isset($_POST['b_return']))
			{
				...
				for($i=0; $i<$rows; $i++)
					if(isset($_POST['cb_book'.$i]))
					{
						...
						$query = $con->prepare("DELETE FROM book_issue_log WHERE member = ? AND book_id = ?;");
						$query->bind_param("ss", $_SESSION['username'], $_POST['cb_book'.$i]);
						if(!$query->execute())
						...
						{
							$penalty = 5*$days;
							$query = $con->prepare("SELECT price FROM book WHERE id_titulo = ?;");
