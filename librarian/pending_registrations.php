			...
			if(isset($_POST['l_confirm']))
			{
				...
				for($i=0; $i<$rows; $i++)
				{
					if(isset($_POST['cb_'.$i]))
					{
						...
						$query = $con->prepare("SELECT username, password, name, email, balance,profile_image FROM pending_registrations WHERE username = ?;");
						$query->bind_param("s", $username);
						$query->execute();
						$row = mysqli_fetch_array($query->get_result());
						
						$query = $con->prepare("INSERT INTO member(username, password, name, email, balance,profile_image) VALUES(?, ?, ?, ?, ?, ?);");
						$query->bind_param("ssssds", $username, $row[1], $row[2], $row[3], $row[4], $row[5]);
						if(!$query->execute())
